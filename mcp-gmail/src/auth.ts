...
async function readGmailSettings(): Promise<GmailSettings> {
...
async function resolveAccount(
  account: string
): Promise<{ acc: StoredAccount; clientId: string; clientSecret: string } | null> {
  ...
  const acc =
    enabled.find(a => a.email === account) ||
    (account === 'default'
      ? enabled[0]
...
export async function isAuthenticated(account: string): Promise<boolean> {
  return (await resolveAccount(account)) !== null;
}

export async function getOAuth2Client(account: string): Promise<OAuth2Client> {
  const resolved = await resolveAccount(account);
  if (!resolved) {
    throw new Error(
      `Account "${account}" is not enabled for MCP. In Cyber Life open Settings → Gmail, connect the account and tick "Claude MCP".`
    );
  }
...
export async function authenticate(
  ...
  return {
    success: false,
    message:
      `Accounts are managed by Cyber Life — open Settings → Gmail, connect "${account}" ` +
