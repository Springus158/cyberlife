...
// GetPromptHistory returns prompt history for a project (newest first, with limit+offset)
func (m *Manager) GetPromptHistory(projectID string, limit, offset int) []PromptHistoryItem {
	m.mu.RLock()
	defer m.mu.RUnlock()

	project, ok := m.state.Projects[projectID]
	if !ok || project.PromptHistory == nil {
		return []PromptHistoryItem{}
	}

	total := len(project.PromptHistory)
	if offset >= total {
		return []PromptHistoryItem{}
	}
	...
	return project.PromptHistory[start:end]
}

// GetPromptHistoryCount returns total count of prompt history items
func (m *Manager) GetPromptHistoryCount(projectID string) int {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	return len(project.PromptHistory)
}

// AddPromptHistory adds a prompt to the project history (prepends, newest first)
func (m *Manager) AddPromptHistory(projectID string, content string) (*PromptHistoryItem, error) {
	m.mu.Lock()
	project, ok := m.state.Projects[projectID]
	if !ok {
		m.mu.Unlock()
	...
	if project.PromptHistory == nil {
		project.PromptHistory = []PromptHistoryItem{}
	}
	...
	project.PromptHistory = append([]PromptHistoryItem{item}, project.PromptHistory...)

	// Cap at 500 items to prevent unbounded growth
	if len(project.PromptHistory) > 500 {
		project.PromptHistory = project.PromptHistory[:500]
	}
...
func (m *Manager) DeletePromptHistory(projectID, itemID string) error {
	m.mu.Lock()
	project, ok := m.state.Projects[projectID]
	if !ok {
		m.mu.Unlock()
	...
	for i, item := range project.PromptHistory {
		if item.ID == itemID {
			project.PromptHistory = append(project.PromptHistory[:i], project.PromptHistory[i+1:]...)
			m.mu.Unlock()
			m.Save()
			return nil
		}
	...
	m.mu.Unlock()
	return fmt.Errorf("prompt history item not found")
}

// ClearPromptHistory clears all prompt history for a project
func (m *Manager) ClearPromptHistory(projectID string) error {
	m.mu.Lock()
	project, ok := m.state.Projects[projectID]
	if !ok {
		m.mu.Unlock()
